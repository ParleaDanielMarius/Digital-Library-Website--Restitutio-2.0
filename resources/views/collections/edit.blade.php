...
@section('title', __('librarian')['edit collection'])
    ...
    <div class="page-header border-bottom">
        <div class="container">
            <div class="d-md-flex justify-content-between align-items-center py-4">
                <h1 class="page-title font-size-3 font-weight-medium m-0 text-lh-lg">{{__('librarian')['edit collection']}}</h1>
{{--                Breadcrumbs--}}
                ...
                <main id="main" class="site-main">
                    <article id="post-6" class="post-6 page type-page status-publish hentry">
                        <header class="entry-header space-top-2 space-bottom-1 mb-2">
                            <h4 class="entry-title font-size-7 text-center">{{__('librarian')['edit collection']}}</h4>
                        </header>
                        <form method="POST" class="checkout woocommerce-checkout row mt-8" action="{{route('collections.update', $collection)}}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
...
{{--                                            Title--}}
                                            <p class="col-12 mb-4d75 form-row form-row-wide validate-required woocommerce-invalid woocommerce-invalid-required-field" id="title_field" data-priority="10">
                                                ...
                                                <input type="text" class="input-text form-control" name="title" id="title" placeholder="Ex: Old Books and Manuscripts" value="{{$collection->title ?? ''}}" autocomplete="title" autofocus="autofocus">
                                            </p>
{{--                                            Description--}}
...
{{--                                            Status--}}
                                            <p class="col-12 mb-4d75 form-row form-row-wide" id="status_field" data-priority="190">
                                                ...
                                                <select name="status" id="status" class="form-control select2-hidden-accessible"
                                                    ...
                                                    <option value="{{Collection::STATUS_ACTIVE}}" @if($collection->status == Collection::STATUS_ACTIVE) selected @endif>{{__('active')}}</option>
                                                    <option value="{{Collection::STATUS_INACTIVE}}" @if($collection->status == Collection::STATUS_INACTIVE) selected @endif>{{__('inactive')}}</option>
                                                </select>
                                            </p>
                                        </div>
                                    </div>
