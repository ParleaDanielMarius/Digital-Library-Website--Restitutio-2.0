...
@section('title', __('librarian')['manage inactive collections'])

@section('content')
    <div class="page-header border-bottom mb-8">
        <div class="container">
            <div class="d-md-flex justify-content-between align-items-center py-4">
                <h1 class="page-title font-size-3 font-weight-medium m-0 text-lh-lg">{{__('librarian')['manage inactive collections']}}</h1>
                {{--                Breadcrumbs--}}
                <nav class="woocommerce-breadcrumb font-size-2">
                    <a href="{{route('home')}}" class="h-primary">{{__('home')}}</a>
                    <span class="breadcrumb-separator mx-1"><i class="fas fa-angle-right"></i></span>
                    <span>{{__('librarian')['manage inactive collections']}}</span>
                </nav>
            </div>
        </div>
    </div>
                        ...
                        <div class="shop-control-bar d-lg-flex justify-content-between align-items-center mb-5 text-center text-md-left">
                            <div class="shop-control-bar__left mb-4 m-lg-0">
                                ...
                                <p class="woocommerce-result-count m-0">
                                    ...
                                    <span>{{ $collections->firstItem() }}</span>
                                    {!! __('pagination')['to'] !!}
                                    <span>{{ $collections->lastItem() }}</span>
                                    {!! __('pagination')['of'] !!}
                            ...
                            <div class="shop-control-bar__right d-md-flex align-items-center">
                                ...
                                <select id="orderBy" name="orderBy" onchange="this.form.submit()" class="js-select selectpicker dropdown-select orderby"
                                        data-style="border-bottom shadow-none outline-none py-2"
                                        data-width="fit">
                                    <option value="5" @if($collections->perPage() == 5) selected @endif>{{__('show')}} 5</option>
                                    <option value="10" @if($collections->perPage() == 10) selected @endif>{{__('show')}} 10</option>
                                    <option value="15" @if($collections->perPage() == 15) selected @endif>{{__('show')}} 15</option>
                                    <option value="20" @if($collections->perPage() == 20) selected @endif>{{__('show')}} 20</option>
                                </select>
                                ...
                                <ul class="products list-unstyled mb-6">
                                    @forelse($collections as $collection)
                                        <li class="product product__list">
                                            <x-collections.collection-list-card :collection="$collection"></x-collections.collection-list-card>
                                        </li>
                                    @empty
                                        {{__('no results')}}
                                    @endforelse
                            ...
                            <div id="widgetHeadingOne" class="widget-head">
                                <a class="d-flex align-items-center justify-content-between text-dark" href="#"
                                   ...
                                   aria-controls="widgetCollapseOne">
                                    <h3 class="widget-title mb-0 font-weight-medium font-size-3">{{__('collections')['title']}}</h3>
