    ...
    <div class="page-header border-bottom">
        <div class="container">
            <div class="d-md-flex justify-content-between align-items-center py-4">
                <h1 class="page-title font-size-3 font-weight-medium m-0 text-lh-lg">{{__('admin')['edit user']}}</h1>
                <nav class="woocommerce-breadcrumb font-size-2">
                    <a href="{{route('home')}}" class="h-primary">{{__('home')}}</a>
                    <span class="breadcrumb-separator mx-1"><i class="fas fa-angle-right"></i></span>{{__('admin')['edit user']}}
                </nav>
            </div>
        </div>
    </div>
...
{{--                                            Status--}}
                                            <p class="col-12 mb-4d75 form-row form-row-wide" id="status_field" data-priority="100">
                                                ...
                                                <select name="status" id="status" class="form-control select2-hidden-accessible"
                                                    ...
                                                    <option value="{{User::STATUS_ACTIVE}}" @if($user->status == User::STATUS_ACTIVE) selected @endif>{{__('active')}}</option>
                                                    <option value="{{User::STATUS_INACTIVE}}" @if($user->status == User::STATUS_INACTIVE) selected @endif>{{__('inactive')}}</option>
                                                </select>
                                            </p>
                                            @error('status')
                                            <p class="text-danger mt-1 col-12 mb-4d75 form-row form-row-wide">{{$message}}</p>
