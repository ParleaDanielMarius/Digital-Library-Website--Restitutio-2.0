...
<div class="product__inner overflow-hidden p-3 p-md-4d875">
    <div class="woocommerce-LoopProduct-link woocommerce-loop-product__link row position-relative">
        <div class="col woocommerce-loop-product__thumbnail mb-3 mb-md-0 d-block " style='max-height: 250px; min-width:68px; display: table'>
            <a href="{{route('collections.show', $collection->slug)}}"><img style='height: 100%;' src="@if($collection->cover_path && file_exists('storage' . '/' . $collection->cover_path)){{asset('storage/' . $collection->cover_path)}} @else{{asset('/assets/img/collections/no-collection-image.png')}}@endif" class="img-fluid d-block mx-auto my-auto attachment-shop_catalog size-shop_catalog wp-post-image img-fluid" alt="image-description"></a>
        </div>
